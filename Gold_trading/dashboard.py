...
Run directly with:  streamlit run dashboard.py
Or imported by app.py and rendered via render().
...
def render() -> None:
    ...
    if page == "Market Overview":
        ...
        series = st.selectbox("Select series", [c for c in f.columns if c in
            ["DXY", "DGS10", "CPI", "CORE_CPI", "CORE_PCE", "FEDFUNDS", "UNRATE",
             "PAYEMS", "INDPRO", "real_yield", "CPI_YoY"]])
        if series:
        ...
        sub = f.tail(lookback)
        for sig_col, conf_col in [("daily_signal", "daily_conf"),
                                  ("weekly_signal", "weekly_conf"),
                                  ("monthly_signal", "monthly_conf"),
                                  ("yearly_signal", "yearly_conf")]:
            st.subheader(sig_col.replace("_signal", "").title())
            st.plotly_chart(
                _chart(sub, sig_col.replace("_signal", "_score"), sig_col),
    ...
    # -----------------------------------------------------------------------
        ...
        st.plotly_chart(_chart(bt_series, "macro_score", f"Macro Score ({horizon})"),
                        use_container_width=True)
